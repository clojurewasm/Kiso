...
function extractCssStr(src: string): string {
  ...
  throw new Error('Expected expanded form with CSS string at position 2: ' + formToString(result));
}

describe('defstyle macro', () => {
  it('expands to su.core/create-stylesheet', () => {
    const result = expandStr(`
      (defstyle my-style
        [:.foo {:color "red"}])
    `);
    const s = formToString(result);
    expect(s).toContain('su.core/create-stylesheet');
